...
def obter_ultimo_sorteio(loteria):
    ...
    conn.close()
    if res: return set(int(x) for x in res[0].split(','))
    return set()
...
def sugerir_jogo(loteria, qtd_jogos, dezenas_por_jogo, usar_ia=False, 
    ...
    limites = {
        "megasena": 60, "lotofacil": 25, "quina": 80, "lotomania": 99, 
        "duplasena": 50, "timemania": 80, "maismilionaria": 50, "diadesorte": 31
    }
    ...
    modelo_ia, cluster_campeao = None, None
    if usar_ia:
    ...
    for _ in range(qtd_jogos):
        ...
        while tentativas < 5000:
            ...
            linhas_count, colunas_count, grid_invalido = {}, {}, False
            for n in jogo:
                r, c = MAPA_GRID[n]
